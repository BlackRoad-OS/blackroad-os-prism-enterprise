...
const disableFlag = String(
  process.env.BR_TEST_DISABLE_DB || process.env.BRC_DISABLE_NATIVE_DB || ''
).toLowerCase();
const SHOULD_DISABLE_DB = disableFlag === '1' || disableFlag === 'true';
...
if (!SHOULD_DISABLE_DB) {
  try {
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.warn('better-sqlite3 unavailable, falling back to mock DB:', err);
    }
  }
}
...
if (!Database) {
  ...
  (function seedRc() {
    const hasPrices = db
      .prepare(
        "SELECT name FROM sqlite_master WHERE type='table' AND name='rc_prices'"
      )
      .get();
    if (hasPrices) {
      const defaults = {
    ...
    const hasCharges = db
      .prepare(
        "SELECT name FROM sqlite_master WHERE type='table' AND name='rc_charges'"
      )
      .get();
    if (hasCharges) {
      const rows = db
        .prepare('SELECT user_id, amount, job_id, reason FROM rc_charges')
        .all();
