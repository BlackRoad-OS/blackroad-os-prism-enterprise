...
function makeSim(N, goal = [N >> 1, N >> 1]) {
  ...
  const s = { N, u, type };
  resetDistances(s, goal);
  return s;
}
function clearWalls(sim) {
  const { N, type } = sim;
  for (let y = 0; y < N; y++)
    for (let x = 0; x < N; x++) if (type[y][x] === 2) type[y][x] = 0;
}
function resetDistances(sim, goal) {
  const { N, u, type } = sim;
  for (let y = 0; y < N; y++)
    for (let x = 0; x < N; x++) u[y][x] = type[y][x] === 2 ? 1e6 : 1e6;
...
function paint(sim, x, y, r, kind) {
  ...
  for (let j = -r; j <= r; j++)
    for (let i = -r; i <= r; i++) {
      ...
      const Y = Math.max(0, Math.min(N - 1, y + j));
      if (i * i + j * j <= r * r) {
        type[Y][X] = kind;
        if (kind === 0 && u[Y][X] === 1e6) u[Y][X] = 1e6;
      }
    }
}
  ...
  function sweep(x0, x1, dx, y0, y1, dy) {
    for (let y = y0; y !== y1; y += dy)
      for (let x = x0; x !== x1; x += dx) {
        ...
        let a = Math.min(ux, uy);
        const b = Math.max(ux, uy);
        let v;
        if (b - a >= 1) v = a + 1;
        else v = (a + b + Math.sqrt(Math.max(0, 2 - (a - b) * (a - b)))) / 2;
        if (v < u[y][x]) u[y][x] = v;
      }
  }
  function get(x, y) {
...
function extractPath(sim, start, goal) {
  ...
  for (let k = 0; k < 4000; k++) {
    ...
    const nx = x - gx / len;
    const ny = y - gy / len; // steepest descent
